...
from fbchat_muqit import (
    Client, Message, ThreadType, State, FBchatException, ThreadLocation
)
...
class Greeg(Client):
    ...
    async def __messaging(self, mid, author_id, message, message_object, thread_id, thread_type, **kwargs):
        if author_id != self.uid:
            await self.__botEvent('type:message', mid=mid, author_id=author_id, message=message,
                                  message_object=message_object, thread_id=thread_id, thread_type=thread_type, **kwargs)
...
async def main():
    ...
    if not os.path.exists(cookies_path):
        print("[ERROR] fbstate.json not found. Please login again to generate a new one.")
        return
    ...
    if await bot.isLoggedIn():
        ...
        try:
            ...
            bot.error(f"Failed to load bot configuration: {e}")
            return
    ...
    try:
        ...
        print("\033[0;91m[BOT] \033[0mAn error occurred while trying to login, please check your bot account or get a new fbstate")
